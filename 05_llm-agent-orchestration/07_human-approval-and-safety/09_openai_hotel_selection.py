"""호텔 후보를 조회한 뒤 사용자 선택을 받아 OpenAI Agent를 재개합니다.

이 예제는 정보가 부족해서 멈추는 ``waiting_user``와 외부 변경 허가를 기다리는
``waiting_approval``이 서로 다른 상태임을 보여줍니다. 호텔 검색은 읽기 작업이며,
사용자의 호텔 선택은 다음 Prompt에 필요한 입력입니다. 실제 예약은 실행하지 않고
선택한 호텔의 취소 정책과 예약 초안까지만 생성합니다.
"""
