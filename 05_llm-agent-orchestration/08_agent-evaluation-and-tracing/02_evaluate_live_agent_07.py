...
before_checks = {
    "승인 대기": paused["status"] == "waiting_approval",
    "승인 전 주문 미실행": change_execution_count(paused) == 0,
}
...
after_checks = {
    "승인 후 완료": completed["status"] == "completed",
    "주문 정확히 한 번 실행": change_execution_count(completed) == 1,
}
