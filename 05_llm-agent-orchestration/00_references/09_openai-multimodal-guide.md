...
이 단원은 05 과정의 필수 멀티 LLM 계약과 분리된 **OpenAI 선택 확장**입니다.
...
| API | 요청 | 응답 |
| --- | --- | --- |
| `POST /api/media/image-analysis` | multipart 이미지와 질문 | `ApiResponse` 안의 `TravelImageAnalysis` |
| `POST /api/media/tts` | JSON 텍스트, 음성, 지시 | `audio/mpeg` |

두 API는 Python Agent와 LangGraph Agent 백엔드에 동일하게 제공합니다.
프런트엔드는 사이드바에서 선택한 백엔드로 요청합니다.
