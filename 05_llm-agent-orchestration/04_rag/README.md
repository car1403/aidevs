...
06 예제는 `00_local-runtime`에서 만든 개별 Container를 실행하고 Embedding 모델을
준비합니다. `C:\mini_agent_st\infra`의 Docker Compose 환경과 동시에 실행하면 같은
...
PDF는 Python 파일과 같은 디렉터리에 `ai.pdf`라는 이름으로 둡니다. 다른 파일이나
질문을 사용하려면 각 예제 위쪽의 `PDF_PATH`, `QUESTION`, `TOP_K`만 수정합니다.
초보 단계에서는 별도의 명령행 인자를 사용하지 않습니다.
