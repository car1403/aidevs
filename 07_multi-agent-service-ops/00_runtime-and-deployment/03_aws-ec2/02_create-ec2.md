...
| --- | --- | --- | --- | --- |
| `01_simple-multi-llm-compose` | `multi-agent-simple-compose` | 수업에서 지정한 사양 | `16 GiB gp3` 이상 | `8501` |
| `05_weather-mcp-deployment-project` | `weather-mcp-deployment` | `t3.small` 권장 | `16 GiB gp3` | `8501` |

`05_weather-mcp-deployment-project`는 Ollama·PostgreSQL·Redis 없이 Frontend, Backend,
Weather MCP 세 Container만 실행합니다. `t3.micro`에서도 실행될 수 있지만 1 GiB 메모리는
Docker Image Build 중 부족할 수 있으므로 초보자 수업에서는 `t3.small`을 권장합니다.
계정의 비용 한도와 강사의 지시가 있다면 그 값을 우선합니다.
...
아닙니다. 예를 들어 `My IP`가 `121.170.161.1/32`이면 해당 Public IP 하나를 허용합니다.
