...
시나리오:
    ...
    Main Thread의 Orchestrator가 결과를 검증해 Shared State에 기록하고, 세 필수 결과가
    모두 성공한 경우에만 Gemma Itinerary Agent를 실행합니다.
...
확인할 내용:
    Worker는 Shared State를 직접 수정하지 않습니다. Orchestrator가 구조화된 Trace와
    성공·실패 목록을 갱신합니다. 정상 흐름에서는 실제 LLM을 최대 4회 호출합니다.
"""
...
JOBS = {
    "weather_agent": (WeatherResult, "날씨 위험과 준비 사항을 정리하세요."),
    "place_agent": (PlaceResult, "대중교통으로 방문할 장소와 선택 근거를 작성하세요."),
...
def distributed_orchestrator_agent() -> CollaborationState:
    ...
    joined_context = {agent_id: state.results[agent_id] for agent_id in JOBS}
